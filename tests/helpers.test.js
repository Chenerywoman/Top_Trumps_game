...
describe('getNames: takes an array of objects and returns a string of the keys, separated by commas', () => {
    const singleCard = [{ 'Sirius Black': 4 }];
    const multipleCards = [{ 'Gilderoy Lockhart': 3 }, { 'George Weasley': 3 },{ 'Rubeus Hagrid': 3 }]

    test('returns a string with a single name given an array with one object', () => {
        expect(getNames(singleCard)).toBe('Sirius Black')
    });
    test('returns a string with a list of items given an array with multiple objects', () => {
        expect(getNames(multipleCards)).toBe('Gilderoy Lockhart, George Weasley, Rubeus Hagrid')
    })
